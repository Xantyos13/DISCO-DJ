...
class Cosmology_nu:
    __slots__ = [
        "_dtype_num", "_dtype",
        "_Omega_c", "_Omega_b", "_h", "_sigma8", "_n_s", "_Omega_k", "_w0", "_wa", "_mnu",
    ...
    def __init__(
        ...
        parameter_keys = ("Omega_c", "Omega_b", "h", "sigma8", "n_s", "Omega_k", "w0", "wa", "mnu")
        parameters = (Omega_c, Omega_b, h, sigma8, n_s, Omega_k, w0, wa, mnu)
    ...
    def __repr__(self):
        ...
        return (
            ...
            f"sigma8={self._sigma8}, n_s={self._n_s}, Omega_k={self._Omega_k}, w0={self._w0}, wa={self._wa}, mnu={self._mnu},"
    ...
    def __str__(self):
        ...
        try:
            ...
            cosmo_str += (
                ...
                f"  mnu: {format_str.format(self._mnu)} "
            )
    ...
    def Omega_nu_rel(self):
        ...
        return self.Omega_gamma * (2.0458496) * (7/8) * (4/11)**(4/3) # Or replace (Neff-1) by 2.0458496 
    ...
    def tree_unflatten(cls, aux_data: dict, children: tuple):
        ...
        """
        num_params = 9  # Number of cosmological parameters
...
class Cosmology:
    ...
    def Omega_nu_rel(self):
        """Radiation term of massless neutrinos"""
        return self.Omega_gamma * (Neff) * (7/8) * (4/11)**(4/3) # Or replace (Neff-1) by 2.0458496 
    ...
    def Omega_de(self):
        """Compute the dark energy density parameter."""
        return 1.0 - self._Omega_c - self._Omega_b - self._Omega_k - self.Omega_gamma - self.Omega_nu_rel
    ...
    def fr0(self):
        """Initial Radiation over Matter fraction"""
        return (self.Omega_gamma + self.Omega_nu_rel) / (self.Omega_b + self.Omega_c)
    # # # # # # # # # # # #
    # Jax PyTree methods
    # # # # # # # # # # # #
    def tree_flatten(self) -> tuple[tuple, dict]:
    ...
    def E(self, a: float | AnyArray) -> AnyArray:
        """Dimensionless Hubble parameter as a function of scale factor."""
        # see https://arxiv.org/pdf/astro-ph/0508156 (Eqs. 3 & 5) for w(a) = w0 + wa (1-a)
        return jnp.sqrt( (self.Omega_gamma + self.Omega_nu_rel)* a ** -4 + self.Omega_m * a ** -3 + self.Omega_k * a ** -2 + self.Omega_de_of_a(a))
    ...
    def Eda(self, a: AnyArray) -> AnyArray:
        ...
        """
        #Radiation term 
        rad_term = (self.Omega_gamma + self.Omega_nu_rel)* a ** -4 
        drad_term = -4* (self.Omega_gamma + self.Omega_nu_rel)* a ** -5 
        ...
        dE2_da = drad_term + dmatter_da + dcurvature_da + dde_da
        E = jnp.sqrt(rad_term + matter_term + curvature_term + de_term)
        return 0.5 * dE2_da / E
